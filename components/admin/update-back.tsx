...
import { SkillData, BackModuleData } from "../../lib/definitions";
...
interface Props{
  ...
  link: string,
  thumbnail?: Blob,
  onSubmit: ( mod: BackModuleData ) => {}
}

export default function UpdateBackMod( props: Props ){
  const { id, name, link, description, onSubmit } = props;
  ...
  const fetchModSkills = async () => {
    await fetch( getURL(`/api/skills/back/${id}`) )
      .then( res => {
        if( res.ok ){
          return res.json();
        }

        throw `Error fetch /api/skills/back/${id} => status (${res.status})`;
      })
      .then( data => {
        setNewSkills( data );
      })
  ...
  useEffect( () => {
    setSubmit(newName !== "" && newLink !== "" && description !== "" && newSkills.length !== 0);
  });
  ...
  <form action={ getURL("/api/front-modules/new") } method="post">
    ...
    <input id="file" name="file" type="text" value={ newLink } onChange={ (event) => setNewLink( event.target.value )} />
