...
export default function AddFront(){
  ...
  const insertModule = async () => {
    const status = fetch( getURL(`/api/front-modules/new`), {
      method: 'POST',
      headers: {
      ...
      .then( res => {
        ...
        switch( res.status ){
          case 501:
            throw `Error with "/api/front-modules/new" API : couldn't insert new module`
            break;
          case 502:
            throw `Error with "/api/front-modules/new" API : couldn't retrieve mod id`
            break;
          case 503:
            throw `Error with "/api/front-modules/new" API : couldn't update skills_f_mod`
            break;
          default:
            throw `Error with "/api/front-modules/new" API : ${res.status}`
        }
      })
      .then( data => {
        if( data.success ){
  ...
  useEffect( () => {
    setSubmit(name !== "" && file !== "" && description !== "" && skillsList.length !== 0);
  });
  ...
  return(
  <AdminLayout title="Add Front-end module">
    <section className={ style.mainSection }>
      ...
      <form action={ getURL("/api/front-modules/new") } method="post">
        ...
        <input id="file" name="file" type="text" value={ file } onChange={ (event) => setFile( event.target.value )} />
